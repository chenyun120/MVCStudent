...
+ (Class)convertedClassForProperty:(NSString *)property
{
    SEL convertSelector = NSSelectorFromString( [NSString stringWithFormat:@"convertPropertyClassFor_%@", property] );
    
    if ( [self respondsToSelector:convertSelector] )
    {
        Class convertedClass;
        NSMethodSignature * sig = [self methodSignatureForSelector:convertSelector];
        NSInvocation *inv = [NSInvocation invocationWithMethodSignature:sig];
        [inv setTarget:self];
        [inv setSelector:convertSelector];
        [inv invoke];
        [inv getReturnValue:&convertedClass];
...
+ (id)objectsWithArray:(id)array
{
    ...
    for ( NSDictionary * obj in array )
    {
        ...
        {
            id convertedObj = [self objectWithDictionary:obj];
            if ( convertedObj ) {
...
+ (instancetype)objectWithDictionary:(NSDictionary *)dictionary
{
    ...
    for ( __unsafe_unretained NSString *property in properties )
    {
        ...
        if ( value )
        {
            ...
            {
                Class convertedClass = [self convertedClassForProperty:property];
                convertedValue = [convertedClass objectsWithArray:value];
            }
            else if ( [value isKindOfClass:[NSDictionary class]] )
            {
                convertedValue = [clazz objectWithDictionary:value];
            }
